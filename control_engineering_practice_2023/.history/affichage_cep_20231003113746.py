...
def chronogrammes_classifications(df_binary, dct_2I, clsfr_shown, wf_name): 
    font = {'family' : 'normal', 'weight' : 'normal', 'size'   : 19}; matplotlib.rc('font', **font)
    ...
    for x, clsfr in enumerate(clsfr_shown):
        ...
        ax.plot(df_binary[clsfr], 'k', linewidth=1)
        plt.ylabel(clsfr)#, plt.grid()
        #ax.get_xaxis().set_visible(False)
        plt.subplots_adjust(wspace = 0, hspace = 0)

        ## coloriage des N2I ##
        # classificateurs multi-résiduels #
        if (clsfr == '$\complement_{\epsilon_{Me}}$') or (clsfr == '$\complement_{\overline{\epsilon}}$') or (clsfr == '$\complement_{MV}$') or (clsfr == '$\complement_{CV}$'):
