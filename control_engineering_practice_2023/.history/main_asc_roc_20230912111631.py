...
for i, sheet in enumerate(sheets):
    #dct_ROC[sheet] = dct_ROC[sheet].drop_duplicates('NUI') # pour éviter les calculs d'aire de trapèze nulles
    ...
    x = dct_ROC[sheet]['NUI']
    y = dct_ROC[sheet]['ADT']
    # nb: pour éviter lec alculs d'aire de trapèze nul (NUI[i-1]=NUI[i]), on définit des bases au moins égales à 1e-2
    AUC = np.sum([(np.max([x.iloc[i-1]-x.iloc[i], 1e-2])*(y.iloc[i]+y.iloc[i-1]))/2 for i in range(1, n)])/(xmax*ADT_max)
