...
for i, sheet in enumerate(sheets):
    dct_ROC[sheet] = dct_ROC[sheet].sort_values(by='NUI', ascending=False)
    ...
    x = dct_ROC[sheet]['NUI']
    y = dct_ROC[sheet]['ADT']
    AUC = np.round(np.sum([(np.max([x.iloc[i-1]-x.iloc[i], 0])*(y.iloc[i]+y.iloc[i-1]))/2 for i in range(1, n)])/(xmax*ADT_max), 3)

    ## ADT(NUI) ##
    if i < 7:
        plt.plot(dct_ROC[sheet]['NUI'], dct_ROC[sheet]['ADT'], 'x', alpha = .8, linestyle='dashed', label=f'{legend[i]}, AUC={AUC}') 
    else:
        plt.plot(dct_ROC[sheet]['NUI'], dct_ROC[sheet]['ADT'], 'o', linestyle='solid', label=f'{legend[i]}, AUC={AUC}') 
    #ax[0].vlines(5, 0, 100, 'r', linewidth=2)
    plt.yticks(ticks = np.round(np.arange(0, ADT_max, 2), 0), labels = np.round(np.arange(0, ADT_max, 2), 0), rotation = 0, ha = 'right')
    plt.xticks(ticks = np.round(np.arange(0, NUI_max, 2), 0), labels = np.round(np.arange(0, NUI_max, 2), 0), rotation = 0, ha = 'right')
    plt.title('$ADT(\overline{N_{UI}})$'), plt.xlabel('$\overline{N_{UI}}$'), plt.ylabel('$ADT$ (days)')
