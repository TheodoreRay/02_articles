...
for i, sheet in enumerate(sheets):
    ...
    ## ADT(NUI) ##
    if i < 7:
        plt.plot(dct_ROC[sheet]['NUI'], dct_ROC[sheet]['ADT'], '.', alpha = .5, linestyle='dashed', label=legend[i]) 
    else:
        plt.plot(dct_ROC[sheet]['NUI'], dct_ROC[sheet]['ADT'], 'o', linestyle='solid', label=legend[i]) 
    #ax[0].vlines(5, 0, 100, 'r', linewidth=2)
    plt.set_yticks(ticks = np.round(np.arange(0, ADT_max, 2), 0), labels = np.round(np.arange(0, ADT_max, 2), 0), rotation = 0, ha = 'right')
    ...
    x = dct_ROC[sheet]['NUI']
    y = dct_ROC[sheet]['ADT']
    AUC = np.round(np.sum([(np.max([x.iloc[i-1]-x.iloc[i], 0])*(y.iloc[i]+y.iloc[i-1]))/2 for i in range(1, n)])/(xmax*ADT_max), 2)
