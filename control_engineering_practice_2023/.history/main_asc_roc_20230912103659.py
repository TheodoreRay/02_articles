...
#%% MAIN
NUI_max = [dct_ROC[sheet]['NUI'].max() for sheet in sheets].max()
ADT_max = [dct_ROC[sheet]['ADT'].max() for sheet in sheets].max()
for i, sheet in enumerate(sheets):
    ...
    x = dct_ROC[sheet]['NUI']
    y = dct_ROC[sheet]['ADT']
    AUC = np.sum([((x[i-1]-x[i])*(y[i]+y[i-1]))/2 for i in range(1, n)])/(NUI_max*ADT_max)
