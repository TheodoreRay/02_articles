...
for sheet in sheets:
    df_ROC, _ = f.import_data_auto(format='excel', sheet=sheet)

    # ADT(NUI) #
    plt.plot(df_ROC['NUI'], df_ROC['ADT'], '.', linestyle='dashed') 
    ...
    plt.set_title('TPR(FPR)'), plt.set_xlabel('FPR(%)'), plt.set_ylabel('TPR(%)'), plt.grid(visible=True), plt.legend(loc='lower right')
