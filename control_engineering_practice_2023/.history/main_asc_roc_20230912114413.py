...
# MAIN
ADT_min = np.min([dct_ROC[sheet]['ADT'].min() for sheet in sheets])
ADT_max = np.max([dct_ROC[sheet]['ADT'].max() for sheet in sheets])
NUI_min = np.min([dct_ROC[sheet]['NUI'].min() for sheet in sheets])
...
for i, sheet in enumerate(sheets):
    ...
    x = dct_ROC[sheet]['NUI']
    y = dct_ROC[sheet]['ADT']
    AUC = np.round(100*np.sum([(np.max([x.iloc[i-1]-x.iloc[i], 0])*(y.iloc[i]+y.iloc[i-1]))/2 for i in range(1, n)])/(xmax*ADT_max), 0)
    ...
    plt.grid(visible=False), plt.legend(loc='lower right')
