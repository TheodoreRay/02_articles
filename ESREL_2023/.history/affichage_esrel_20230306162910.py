...
def matrix(data, classe = ''):
    ...
    #scatter_matrix(data[colonnes].sample(n = 2000).dropna(), alpha = 0.5, figsize = (4, 4), grid = True, diagonal = 'kde')
    ## matrice de corrélations ##
    plt.figure('correlation matrix')
    #cmap = sns.diverging_palette(230, 20, as_cmap=True)
    cmap = 'RdYlGn'
    #mask = np.triu(np.ones_like(data[colonnes].corr()['temp_huile_multi'], dtype=bool))
    df=pd.DataFrame(data[colonnes].corr()['temp_huile_multi']).T
    sns.heatmap(df, annot=True, cmap=cmap, vmin=0, vmax=1, center=0.5, square=True, linewidths=.5, cbar_kws={"shrink": .5})
